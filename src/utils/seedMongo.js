...
async function seedMongoDB() {
  try {
    ...
    // Hanya lakukan seed otomatis saat database BENAR-BENAR KOSONG PERTAMA KALI (User count === 0)
    if (userCount === 0) {
      await User.insertMany(INITIAL_USERS);
      await KategoriProduk.insertMany(INITIAL_KATEGORI_PRODUK);
      await KategoriBahanBaku.insertMany(INITIAL_KATEGORI_BAHAN);
      await BahanBaku.insertMany(INITIAL_BAHAN);
      await Produk.insertMany(INITIAL_PRODUK);
      await Resep.insertMany(INITIAL_RESEP);
      await AuditLog.insertMany(INITIAL_LOGS);
      console.log('🌱 Initial Seed Database inserted to MongoDB Atlas!');
    }

    console.log('✨ MongoDB Atlas database "saren_one_db" ready!');
  } catch (err) {
